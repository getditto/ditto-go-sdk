...
_examples/
  ...
  go.sum
  sync-test/                # Real-time data synchronization
    main.go
  presence-test/            # Peer presence and discovery
    main.go
  transport-diagnostics-test/ # Network transport monitoring
    main.go
  auth-test/                # Authentication and logout
    main.go
  execute-test/             # DQL Execute operations
    main.go
  transaction-test/         # Transaction operations
    main.go
```
...
### 1. sync-test - Real-time Data Synchronization
...
5. Export logs to `./ditto-go-example-sync-test-logexport.jsonl.gz`
...
### 2. presence-test - Peer Presence and Discovery
...
7. Export logs to `./ditto-go-example-presence-test-<instanceID>-logexport.jsonl.gz`
...
- `-persistence-directory <string>`: Directory for Ditto persistence (defaults to /tmp/ditto-go-example-presence-test-<id>)
- `-log-export-path <string>`: Path for exported log file (defaults to ./ditto-go-example-presence-test-<id>-logexport.jsonl.gz)

### 3. transport-diagnostics-test - Network Transport Monitoring
...
6. Export logs to `./ditto-go-example-transport-diagnostics-test-logexport.jsonl.gz`
...
### 4. auth-test - Authentication and Logout
...
### 5. execute-test - DQL Execute Operations
...
The execute-test example demonstrates:
...
### 6. transaction-test - Transaction Operations
...
The transaction-test example demonstrates:
...
- **sync-test**: `/tmp/ditto-go-example-sync-test/`
- **presence-test**: `/tmp/ditto-go-example-presence-test-<instanceID>/`
- **transport-diagnostics-test**: `/tmp/ditto-go-example-transport-diagnostics-test/`
- **auth-test**: `/tmp/ditto-go-example-auth-test/`
- **execute-test**: `/tmp/ditto-go-example-execute-test/`
- **transaction-test**: `/tmp/ditto-go-example-transaction-test/`
...
- [Ditto Go SDK API Reference](https://pkg.go.dev/github.com/getditto/ditto-go-sdk)
